      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="list-link mb-4">
            ...
            <a href="/admin/customers/refresh" class="btn btn-info" style="position: relative;"><i class="fas fa-sync-alt"></i> Refresh Data <div class="bg-success d-flex justify-content-center align-items-center" style="position: absolute; width: 20px; height: 20px; border-radius: 50px; right: -10px; top: -10px;">{{ count($state ?? []) != 0 ? count($state ?? [])  - count($customers ?? []) : '0'  }}</div></a>
          </div>
...
<script>
  ...
  Livewire.on('refreshState', function() {
    setTimeout(() => {
      Livewire.emit('getState');
    }, 12000)
  });
